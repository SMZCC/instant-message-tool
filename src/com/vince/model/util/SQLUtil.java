/**
 * @author 流浪大法师
 * @time 2016-2-14 下午11:37:38
 * @email liuliangsir@gmail.com
 * @descript 自动生成对象的增删改查SQL语句的通用方法工具类
 * @warning 注意将字符集设置成UTF-8
 */
package com.vince.model.util;
...
import com.vince.controller.util.DateUtil;
		...
		try {
			for (int i = 0; i < fieldList.size(); i++) {
				...
				if("class java.util.Date".equals(field.getType().toString())
						&& field.get(obj) != null){
					String timeStr = DateUtil.format((Date)field.get(obj), "yyyy-MM-dd HH:mm:ss");
		...
		for(int i = 0; i < fields.length; i++){
			...
			if(!fields[i].getName().equalsIgnoreCase(keyColumn)){ // 非主键列记录插入SQL语句的参数占位符
				...
				if("class java.util.Date".equals(fields[i].getType().toString())
					...
					String timeStr = DateUtil.format((Date)fields[i].get(obj), "yyyy-MM-dd HH:mm:ss");
					paramStrBuf.append("to_date(?, 'yyyy-MM-dd HH24:mi:ss')");
					paramValues.add(timeStr);
					// 记录对象属性的数据类型
